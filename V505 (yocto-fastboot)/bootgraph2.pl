...
my $mult = 5000;
my $mh_width = ($maxtime - $firsttime) * $mult * 1.007;
print "<svg width=\"$mh_width\" height=\"1000\" version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\">\n";
...
foreach my $key (@initcalls) {
	...
	if ($duration >= $threshold) {
		...
		if ($type{$key} == 1) {
			$y = $y + 15;
			print "<rect x=\"$s\" width=\"$w\" y=\"$y\" height=\"325\" style=\"$style_wait\"/>\n";
		} else {
			print "<rect x=\"$s\" width=\"$w\" y=\"$y\" height=\"325\" style=\"$style\"/>\n";
			if ($duration >= 0.02) {
				print "<text transform=\"translate($s2,$y2) rotate(90)\" font-size=\"30pt\">$key</text>\n";
			} elsif ($duration > 0.005) {
				print "<text transform=\"translate($s3,$y2) rotate(90)\" font-size=\"18pt\">$key</text>\n";
			}
		}
	}
}
...
while ($time < $maxtime) {
	...
	print "<text transform=\"translate($s3,89) rotate(0)\" font-size=\"30pt\">$tm</text>\n";
	$time = $time + 0.1;
}
